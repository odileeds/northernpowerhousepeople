...
function genderSplitByRole(typ,order){
	...
	fl = 0;
	tl = 0;
	ll = '<ul class="people" style="display:none;">';
	var nrow = 0;
	for(var o=0; o < order.length;o++){
		t = 0;
		r = order[o];
		if(roles[r]){
			for(var g in roles[r]){
				t += roles[r][g];
				tl += roles[r][g];
			}
			str += '<tr><td>'+r+'</td>';
			...
			for(var p = 0; p < people[r].length; p++){
				...
				ll += '<li class="'+people[r][p].gender+'">'+people[r][p].name+' - '+people[r][p].role+', '+(people[r][p].org.url ? '<a href="'+people[r][p].org.url+'">' : '')+people[r][p].org.name+(people[r][p].org.url ? '</a>':'')+')</li>';
			}
			list += '</ul>';
	...
	ll += '</ul>';
	if(nrow > 1){
		var m = (100*ml/tl).toFixed(1);
		var f = (100-parseFloat(m)).toFixed(1);
		str += '<tr class="average"><td>Total</td>';
		str += '<td><div class="chart" style="cursor:pointer;">'+(fl == 0 ? '' : '<div class="bar female" style="width:'+f+'%;" title="'+f+'%"><span>'+fl+' '+(fl == 1 ? 'woman':'women')+'</span></div>')+'<div class="bar male" style="width:'+m+'%;" title="'+m+'%"><span>'+ml+' '+(ml == 1 ? 'man':'men')+'</span></div></div>'+ll+'</td>';
		str += '</tr>';
	}
	str += '</table>';
